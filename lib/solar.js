...
export {getSolarData, hra, getPoint}
...
function getPoint(angle, radius, centerX, centerY){
    angle = angle +180;
    let x = 2*centerX-(centerX + radius * Math.sin(degToRad(-(angle % 360))));
    let y = centerY + radius * Math.cos(degToRad(-(angle % 360)));
    return {
        x:x,
        y:y,
        angle:angle - 180
